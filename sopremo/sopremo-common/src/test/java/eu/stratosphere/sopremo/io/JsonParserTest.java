...
public class JsonParserTest {
	...
	public static List<Object[]> combinations() {
		return Arrays.asList(new Object[][] {
			{ " [42] ", new ArrayNode<IJsonNode>().add(IntNode.valueOf(42)), 1 },
			{ "42", IntNode.valueOf(42), 1 },
			{ "[null]", new ArrayNode<IJsonNode>().add(NullNode.getInstance()), 1 },
			{ "null", NullNode.getInstance(), 1 },
			{ "[null,null]", new ArrayNode<IJsonNode>().add(NullNode.getInstance()).add(NullNode.getInstance()), 1 },
			{ "[true]", new ArrayNode<IJsonNode>().add(BooleanNode.TRUE), 1 },
			{ "false", BooleanNode.FALSE, 1 },
			{ "[42.42]", new ArrayNode<IJsonNode>().add(DecimalNode.valueOf(BigDecimal.valueOf(42.42))), 1 },
			{ String.valueOf("[" + String.valueOf(Long.valueOf(Integer.MAX_VALUE) + 1) + "]"),
				new ArrayNode<IJsonNode>().add(LongNode.valueOf(Long.valueOf(Integer.MAX_VALUE) + 1)), 1 },
			...
			{ "{\"key1\" : null}", new ObjectNode().put("key1", NullNode.getInstance()), 1 },
			{
				...
				new ObjectNode().put(
					"key1",
					new ArrayNode<IJsonNode>().add(IntNode.valueOf(1)).add(IntNode.valueOf(3)).add(
